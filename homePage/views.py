...
def comprarPage(request):
    if request.method == "POST":
        if request.POST.get("Comando") == "EnviarSMS":
        ...
        elif request.POST.get("Comando") == "Verificar":
            print("Boton verificar presionado")
            celular = request.POST.get("celular")
            codigoValidacionIngresado = request.POST.get("codigo")
            responseData = {"Mensaje": "El codigo ingresado no es valido"}
            if buscarCodigoEnBaseDatos(celular, codigoValidacionIngresado, 1):
                print("Codigo Correcto!!!")
                responseData = {"Estado": "Valido", "Mensaje": "Su celular ha sido validado"}
