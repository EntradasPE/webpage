...
function enviarSMS() {
    Swal.fire({
        title: 'Ingrese codigo de validacion',
        input: 'text',
        inputAttributes: {
            autocapitalize: 'off'
        },
        ...
        preConfirm: (codigo) => {
            return fetch(`comprar/`, {
                method: 'POST', // Puedes usar 'POST' u 'GET' según tus necesidades
                body: JSON.stringify({
                    Comando: 'EnviarSMS',
                    codigo: codigo,
    ...
    //     PopupMensaje("error", "Error", "El número de celular debe tener 9 dígitos.");
    //     return false;
    // }
    ...
    //             PopupMensaje("info",
    //                 "Verificacion en proceso", "Un codigo de validacion se le envio a su celular")
    //             SMSenviado = true;
    //         }
    //         else {
    //             PopupMensaje("error",
...
function enviarCodigo() {
    ...
    if (document.getElementById("codigo").value.trim().length !== 8) {
        PopupMensaje("error", "Error", "El codigo de verificacion debe tener 8 caracteres")
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.Estado == "Valido") {
                PopupMensaje("success", "Verificado", response.Mensaje)
                validado = true
            }
            else {
                PopupMensaje("error", "Error", response.Mensaje)
                onetime = false
            }
        },
        error: function (xhr, status, error) {
...
function verificarDatosCompletos() {
    if (!validado) {
        PopupMensaje("error", "Error", "No ha validado su celular")
        return false
    }
    if (document.getElementById("dni").value.length < 8) {
        PopupMensaje("error", "Error", "Falta ingresar su documento")
        return false
    }
    if (document.getElementById("nombre").value.length < 3) {
        PopupMensaje("error", "Error", "Falta ingresar su nombre completo")
        return false
    }
    if (document.getElementById("pin").value.length < 6) {
        PopupMensaje("error", "Error", "Falta ingresar su pin (6 digitos minimo)")
        return false
    }

    if (CaracteresRepetidosConsecutivos(document.getElementById("pin").value.toString())) {
        PopupMensaje("error", "Error", "El PIN no puede tener numeros repetidos o consecutivos (111) o (1234)")
        return false;
    }

    if (!document.getElementById("Privacidad").checked) {
        PopupMensaje("error", "Error", "Debe aceptar la politica de privacidad")
        return false
    }
    ...
    PopupMensaje("success", "Guardado", "Sus datos han sido guardados exitosamente")
    return true
}
...
function verificarDatosOpcional() {
    if (document.getElementById("correo").value.length < 12 || !document.getElementById("correo").value.toString().includes('@') || !document.getElementById("correo").value.toString().includes('.')) {
        PopupMensaje("error", "Error", "Debe ingresar un correo electronico valido")
        return
    }

    if (document.getElementById("correo2").value != document.getElementById("correo").value) {
        PopupMensaje("error", "Error", "Los correos no coinciden")
        return
    }

    if (document.getElementById("respuesta1").value.length < 3) {
        PopupMensaje("error", "Error", "Debe responder la primera pregunta de seguridad")
        return
    }

    if (visibles >= 1 && document.getElementById("respuesta2").value.length < 3) {
        PopupMensaje("error", "Error", "Debe responder la segunda pregunta de seguridad")
        return
    }

    if (visibles >= 2 && document.getElementById("respuesta3").value.length < 3) {
        PopupMensaje("error", "Error", "Debe responder la tercera pregunta de seguridad")
        return
    }
